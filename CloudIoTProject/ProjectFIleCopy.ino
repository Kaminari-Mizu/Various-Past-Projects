...
static String telemetry_payload1 = "{}";
static String telemetry_payload2 = "{}";
...
static void generateTelemetryPayload1()
{
  ...
  distance1 = readUltrasoundSensor(trig1, echo1);
  //distance2 = readUltrasoundSensor(trig2, echo2);
  Serial.println(distance1);
  //Serial.println(distance2);
  telemetry_payload1 = "{ \"msgCount\": " + String(distance1) + " }";

  sendTelemetry(telemetry_payload1);
}

static void generateTelemetryPayload2()
{
  // You can generate the JSON using any lib you want. Here we're showing how to do it manually, for simplicity.
  // This sample shows how to generate the payload using a syntax closer to regular delevelopment for Arduino, with
  // String type instead of az_span as it might be done in other samples. Using az_span has the advantage of reusing the 
  ...
  Serial.println(distance2);
  //Serial.println(distance2);
  telemetry_payload2 = "{ \"msgCount\": " + String(distance2) + " }";

  sendTelemetry(telemetry_payload2);
}
...
void setup() { 
  ...
  rudder.write(90);
  Serial.begin(9600);
  //maxDist1 = calibration(trig1, echo1);
  //maxDist2 = calibration(trig2, echo2);
  establishConnection(); }
...
void loop()
{
  ...
  {
    //sendTelemetry();
    generateTelemetryPayload1();
    generateTelemetryPayload2();
  ...
  /*else if(distance2 >= (maxDist2-200) && distance1 <(maxDist1-200)){
  	rudder.write(135);
  }
  else if(distance1>=(maxDist1-200) && distance2 >= (maxDist2-200)){
  	rudder.write(180);
  }*/
  else{
    rudder.write(90);
